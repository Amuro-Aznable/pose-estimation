...
# def read_keypoints(filename_txt):
#     num_keypoints = 25
#     kpts_xyz = np.loadtxt(filename_txt)
#     kpts_xyz = kpts_xyz.reshape(-1, num_keypoints, 3)
#     return kpts_xyz
...
def filter_smoothing(kpts, filter='median'):
    ...
    for joint in filtered['joints']:
        ...
        if filter == 'median':
            ...
            b, a = b, a = butter(N=butterworth_order / 2,
...
def get_base_skeleton(kpts, normalization_bone='neck'):
    ...
    def _set_length(joint_type):
        base_skeleton['left' +
                      joint_type] = offset_directions['left' + joint_type] * (
                          (body_lengths['left' + joint_type] +
                           body_lengths['right' + joint_type]) /
                          (2 * normalization))
        base_skeleton['right' +
                      joint_type] = offset_directions['right' + joint_type] * (
                          (body_lengths['left' + joint_type] +
                           body_lengths['right' + joint_type]) /
                          (2 * normalization))
    ...
    # _set_length('hand')
    # _set_length('thumb')
    base_skeleton['neck'] = offset_directions['neck'] * (body_lengths['neck'] /
                                                         normalization)
    base_skeleton['neckup'] = offset_directions['neckup'] * (
        body_lengths['neckup'] / normalization)
    base_skeleton['head'] = offset_directions['head'] * (body_lengths['head'] /
                                                         normalization)
    base_skeleton['waist'] = offset_directions['waist'] * (
        body_lengths['waist'] / normalization)
...
def get_hips_position_and_rotation(frame_pos,
                                   root_joint='hips',
                                   root_define_joints=['lefthip', 'neck']):
    """
...
def get_joint_rotations(joint_name, joints_hierarchy, joints_offsets,
    ...
    for i, parent_name in enumerate(joints_hierarchy[joint_name]):
        ...
        R = utils.get_R_z(_r_angles[0]) @ utils.get_R_x(
            _r_angles[1]) @ utils.get_R_y(_r_angles[2])
        _invR = _invR @ R.T
...
def get_rotation_chain(joint, hierarchy, frame_rotations):
    ...
    for parent in hierarchy:
        ...
        _R = utils.get_R_z(angles[0]) @ utils.get_R_x(
            angles[1]) @ utils.get_R_y(angles[2])
        R = R @ _R
        ...
        while (depth <= max_connected_joints):
            for joint in kpts['joints']:
                if len(kpts['hierarchy'][joint]) == depth:
                    joint_rs = get_joint_rotations(joint, kpts['hierarchy'],
    ...
    for framenum in range(kpts['lefthip'].shape[0]):
        ...
        for _j in kpts['joints']:
            ...
            ax.text(r2[0],
                    ...
                    r2[2],
                    '%s' %
                    (str(np.round(frame_rotations[_j] * 180 / np.pi, 2))),
                    size=7,
                    zorder=1,
                    weight='bold',
                    color='k')
...
def calculate_skeleton_from_joint_angles(kpts, plotting=False, fps=0):
    ...
    for framenum in range(nframes):
        ...
        for _j in kpts['joints']:
            ...
            if _j == 'hips':
                kpts_from_angles[_j][
                    framenum] = kpts['hips'][framenum] / kpts['normalization']
                continue
            ...
            for parent in hierarchy:
                ...
                R = get_rotation_chain(parent, kpts['hierarchy'][parent],
                                       frame_rotations)
            ...
            r2 = r1 + get_rotation_chain(
                hierarchy[0], hierarchy,
                frame_rotations) @ kpts['base_skeleton'][_j]
...
if __name__ == '__main__':
    ...
    # # Load the txt files
    # file_list = [
    #     fname for fname in os.listdir(input_dir) if fname.endswith('.txt')
    # ]

    # for fname in file_list:
    #     # Load the pose data from the txt file
    #     with open(os.path.join(input_dir, fname), 'r') as file:
    #         lines = file.readlines()

    #     # Parse the data to extract 25 three-dimensional points
    #     kpts = []
    #     for line in lines:
    ...
    #     # Now you can process the keypoints as needed or save them to the output directory
    ...
    #     output_kpts = calculate_skeleton_from_joint_angles(kpts,
    #                                                        plotting=True,
    #                                                        fps=30)
    #     a = 0
    ...
    # load the pose file
    file_list = [
        fname for fname in os.listdir(input_dir) if fname.endswith('.npy')
    ]
    ...
    for fname in tqdm(file_list):
        ...
        # draw the skeleton
        # draw_skeleton_from_joint_coordinates(filtered_kpts_assign)
        output_kpts = calculate_skeleton_from_joint_angles(filtered_kpts,
                                                           plotting=True,
                                                           fps=30)
        a = 0
